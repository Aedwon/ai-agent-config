...
def diff(root: Path, adapter_id: str, target_root: Path) -> str:
    ...
    if target_path.exists() and not target_path.is_file():
        raise ConfigError("target path is not a regular file: {}".format(adapter.output_path))
    try:
    ...
    with tempfile.TemporaryDirectory(prefix="ai-agent-config-diff-") as directory:
        rendered_path = render(source_root, adapter_id, Path(directory))[0]
    ...
    return "".join(
        difflib.unified_diff(
            ...
            fromfile="target/{}".format(adapter.output_path),
            tofile="rendered/{}".format(adapter.output_path),
        )
    )
